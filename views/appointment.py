...
def show():
    ...
    st.markdown(
        ...
        <div class="appointment-shell">
            ...
            <p class="appointment-subtitle">
                Schedule a reminder and confirm the contact number to receive notifications.
            </p>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    # Restrict timing to between 9AM and 5PM
    if (appointment_time < time(9, 0) or appointment_time > time(17, 0)):
        st.error("Appointments are only between 9AM and 5PM")
        return
    ...
    if phone_choice == "Use saved phone number":
        ...
        if not phone_number:
            st.warning("No saved phone number was found for this patient. "
        ...
        phone_number = st.text_input(
            "New Phone Number",
            value="",
            placeholder="e.g. 12345678 or +65 1234 5678"
        )

    if "appointment_loading" not in st.session_state:
        st.session_state.appointment_loading = False

    if "appointment_error" not in st.session_state:
        st.session_state.appointment_error = None
    ...
    if st.session_state.appointment_loading:

        st.button(
            "LOADING APPOINTMENT...",
            key="appointment_loading_btn",
        ...
        with st.spinner("Processing appointment..."):
            ...
            try:
                if not phone_number.strip():
                    st.session_state.appointment_loading = False
                    st.session_state.appointment_error = (
                ...
                print(
                    "No-show probability (for debugging):",
                    prediction["probability"]
                )

                print(
                    "Risk level (for debugging):",
                    prediction["risk"]
                )
                ...
                if sent:
                    ...
                    st.session_state[
                        ...
                        "Appointment booked! "
                        "You'll receive a reminder 30 minutes "
                    ...
                    st.session_state.appointment_error = (
                        ...
                        "but the reminder could not be sent."
                    )
                    st.rerun()
        ...
        if st.session_state.appointment_error:
            st.error(st.session_state.appointment_error)
            st.session_state.appointment_error = None

        if st.button(
            "CONFIRM APPOINTMENT",
            key="confirm_appointment_btn",
            use_container_width=True
        ):
            st.session_state.appointment_loading = True
            st.rerun()
    footer()